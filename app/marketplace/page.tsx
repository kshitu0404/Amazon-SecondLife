...
import { ShieldCheck, Leaf, Filter, RefreshCw, Star, ShoppingCart, ArrowRight } from 'lucide-react';
...
function MarketplaceContent() {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (saved) {
        try {
          const customProduct = JSON.parse(saved);
          if (!list.some((p) => p.id === customProduct.id)) {
            list.unshift(customProduct);
          }
        } catch (e) {
          console.error(e);
        }
                  ...
                  >
                    ...
                    <div className="pt-4 mt-3 border-t border-slate-100 flex flex-col text-left">
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-3.5">
                        <Link
                          href={`/health-card?id=${product.id}`}
                          className="col-span-1 bg-slate-50 hover:bg-slate-100 text-slate-800 text-[11px] font-bold py-2 px-1 rounded border border-slate-300 text-center transition shadow-xs"
                        >
                          Passport
                        </Link>
